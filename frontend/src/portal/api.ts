...
// ─── Epic B — Customer Invite ────────────────────────────────────────────

export interface InviteTask {
  id: number;
  name: string;
  target_channel: string;
  status: 'pending' | 'running' | 'paused' | 'paused_no_funds' | 'completed' | 'failed';
  total_count: number;
...
export interface InviteCostPreview {
  target_count: number;
  unit_price_cents: number;
  estimated_total_cents: number;
  balance_cents: number;
...
export const inviteApi = {
  previewCost: (target_count: number) =>
    portalApi.post<InviteCostPreview>('/customer/invite/preview-cost',
      { target_count }).then(r => r.data),
  create: (data: CreateInviteRequest) =>
    portalApi.post<InviteTask>('/customer/invite/tasks', data).then(r => r.data),
  list: (params: { status?: string; skip?: number; limit?: number } = {}) =>
    portalApi.get<InviteTask[]>('/customer/invite/tasks', { params }).then(r => r.data),
  get: (id: number) =>
    portalApi.get<InviteTask>(`/customer/invite/tasks/${id}`).then(r => r.data),
  logs: (id: number, params: { skip?: number; limit?: number } = {}) =>
    portalApi.get<InviteLog[]>(`/customer/invite/tasks/${id}/logs`, { params }).then(r => r.data),
  pause: (id: number) =>
    portalApi.post<InviteTask>(`/customer/invite/tasks/${id}/pause`, {}).then(r => r.data),
  resume: (id: number) =>
    portalApi.post<InviteTask>(`/customer/invite/tasks/${id}/resume`, {}).then(r => r.data),
  cancel: (id: number) =>
    portalApi.delete<InviteTask>(`/customer/invite/tasks/${id}`).then(r => r.data),
};
...
// ─── Epic B — Customer Invite (group pull) ───────────────────────────────
...
export interface CreateInviteTaskRequest {
  name: string;
  target_channel: string;
  account_ids?: number[];
  target_user_ids?: number[];
...
export const inviteApi = {
  previewCost: (target_count: number) =>
    portalApi.post<InviteCostPreview>('/customer/invite/preview-cost',
      { target_count }).then(r => r.data),
  create: (data: CreateInviteTaskRequest) =>
    portalApi.post<InviteTask>('/customer/invite/tasks', data).then(r => r.data),
  list: (params: { status?: string; skip?: number; limit?: number } = {}) =>
    portalApi.get<InviteTask[]>('/customer/invite/tasks', { params }).then(r => r.data),
  get: (id: number) =>
