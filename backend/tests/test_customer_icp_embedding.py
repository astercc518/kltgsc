...
def test_set_icp_text_whitespace_only_clears():
    """全空白 → 视为清空"""
    fake_session = MagicMock()
...
def test_set_icp_text_trims_leading_trailing():
    fake_session = MagicMock()
    fake_customer = MagicMock(id=1, icp_profile_text=None)
    fake_session.get.return_value = fake_customer
    with patch(
        "app.services.customer_icp_service.embed_text", return_value=[0.1]*768,
    ):
        set_customer_icp_text_and_embed(
