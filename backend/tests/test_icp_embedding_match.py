...
@pytest.mark.asyncio
async def test_layer2_passes_when_above_threshold():
    """模拟相似度 0.6 > 阈值 0.55 → PASS"""
    icp_vec = [1.0, 0.0, 0.0] + [0.0] * 765
    msg_vec = [0.9, 0.1, 0.0] + [0.0] * 765
    fake_session = MagicMock()
    with patch(
        "app.services.lead_detector.embed_text", new=AsyncMock(return_value=msg_vec),
    ):
        result = await layer2_icp_similarity(
...
@pytest.mark.asyncio
async def test_layer2_blocks_below_threshold():
    """相似度 0.3 < 0.55 → block"""
...
@pytest.mark.asyncio
async def test_layer2_degrades_when_icp_embedding_missing():
...
@pytest.mark.asyncio
async def test_layer2_degrades_when_message_embed_fails():
    """消息 embedding 服务挂 → degrade pass (不阻塞管线)"""
    icp_vec = [1.0] * 768
    fake_session = MagicMock()
    with patch(
        "app.services.lead_detector.embed_text", new=AsyncMock(return_value=None),
    ):
        result = await layer2_icp_similarity(
...
@pytest.mark.asyncio
async def test_layer2_borderline_flag():
...
@pytest.mark.asyncio
async def test_layer2_zero_vector_returns_zero_similarity():
